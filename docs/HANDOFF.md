...
| `spoof_classifier.py` | **New 2026-09-08.** `SpoofClassifier` ABC, split out to avoid a circular import between `detection.py` and `aasist.py` ✅ |
| `detection.py` | `HeuristicClassifier` — **unvalidated**, see DEF-1 — plus `_select_active_classifier()`, which picks AASIST-L or the heuristic once at process start ✅ |
| `aasist.py` | **New 2026-09-08.** `AASISTLClassifier` — the official AASIST-L ONNX checkpoint via ONNX Runtime (CPU), now the default primary detector behind D-1's `SpoofClassifier` interface. See DEV-9 below ✅ |
...
**Tests**, all re-run 2026-09-08 after the AASIST-L integration (see DEV-9):

| Suite | Command | Result |
|---|---|---|
| Go | `go -C gateway test ./...` | **all packages ok** |
| Python | `python -m pytest backend/tests -q` | **39 passing** (16 pre-existing + 23 new in `test_aasist.py`) |
| Frontend | `npm run build` (in `frontend/`) | **builds clean** |
...
| DEV-9 | D-1 named **Qwen2-Audio + LoRA/QLoRA** as the eventual real detector; the interim/actual detector deployed 2026-09-08 is **AASIST-L** (ONNX, CPU, ~85k params) instead | AASIST-L is public, needs no training run, meets the latency budget on CPU (unlike D-1's projected >800 ms), and needs no PyTorch/torchaudio in the sidecar's default dependency set. It is not fine-tuned on this project's data: model card numbers are 0.99% EER in-domain (ASVspoof2019 LA) vs. 44.45% EER out-of-domain (InTheWild) — treat it as one unvalidated-for-this-deployment signal, not a finished replacement for D-1's training plan. `calibrator_version = identity-sigmoid@0.0.0-unvalidated` (CLAUDE.md invariant 10 is not yet satisfied; see `05-ML_MODEL_LIFECYCLE.md` §3). The heuristic remains available as `AASIST_ENABLED=false` or as an automatic fallback on load/inference failure. | `05` §1, §3; CLAUDE.md D-1 |
...
| Date | Change |
|---|---|
| 2026-09-08 | **AASIST-L integrated as the primary spoof detector** (DEV-9). New: `backend/app/spoof_classifier.py` (the `SpoofClassifier` ABC, split out of `detection.py` to avoid a circular import), `backend/app/aasist.py` (`AASISTLClassifier` — ONNX Runtime, CPU, the official [SpeechAntiSpoofingBenchmarks/AASIST-L](https://huggingface.co/SpeechAntiSpoofingBenchmarks/AASIST-L) checkpoint scored on the raw pre-filter 16 kHz mono window, deterministic 64,600-sample crop/tile matching upstream `clovaai/aasist` eval behaviour, sigmoid-mapped uncalibrated score), `backend/tools/download_aasist_model.py` (one-shot SHA-256-verified fetch, stdlib-only, no `huggingface_hub` dependency), `backend/models/README.md` (the model file itself stays gitignored). `detection.py` now selects AASIST-L or the heuristic once at process start (`AASIST_ENABLED`, default true) and keeps the heuristic as an always-available fallback, including a per-window `try/except` in `sidecar.analyse_buffer` so a detector exception degrades that one window rather than the request. `sidecar.py`'s `/internal/health` and the gateway's `/api/v1/health` gained additive `detector_mode`/`detector_provider` fields. New dependency: `onnxruntime`. New tests: `backend/tests/test_aasist.py` (23 tests: padding/cropping, dtype, NaN/Inf and malformed-output rejection, load failure and idempotent loading, score-direction mapping, fallback behaviour, health reporting, and a real-model smoke test skipped when the file isn't present). Two of the DEF-1 regression tests in `test_pipeline.py` that asserted `HeuristicClassifier`-specific numeric behaviour through the shared `classifier` singleton now instantiate `HeuristicClassifier()` directly, since that singleton can now be a different detector — the other DEF-1 regression test (`test_phase0_detector_output_would_reach_high_in_the_gateway`) still passes generically against whichever detector is active. Suites: **all Go packages ok**, **39 Python** (16 pre-existing + 23 new), frontend builds clean. Measured on the integration machine: AASIST-L CPU inference ~140 ms median per 3 s window after warm-up (see the AASIST-L section in `README.md` for the full diagnostic). Real end-to-end run against `fixture-steady.wav`: risk 92.6 HIGH → BLOCK, ledger chain verified. Live Asterisk telephony audio reaches this same detector automatically once it reaches the sidecar at all (unchanged from before this integration) — whether Asterisk itself is up is tracked separately in §5, not by this change. |
